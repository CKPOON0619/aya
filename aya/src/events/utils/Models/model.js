...
const model = tf.sequential();
  ...
  model.add(tf.layers.dense({units: 1000, activation: 'sigmoid'}));//1000 prms
  model.add(tf.layers.dropout({}));
  model.add(tf.layers.batchNormalization({center:true}))//1000+2000
  model.add(tf.layers.dense({units: 500, activation: 'relu'}));//3000+500
  model.add(tf.layers.dropout({}));
  model.add(tf.layers.batchNormalization({}))//3500+1000
  model.add(tf.layers.dense({units: 100, activation: 'relu'}));//4500+100
  model.add(tf.layers.dropout({}));
