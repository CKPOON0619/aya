...
function handleModelUpload() {
  ...
  try{
    tf.loadModel(
     state.modelFiles[1].type==='application/json'?tf.io.browserFiles([state.modelFiles[1], state.modelFiles[0]]):tf.io.browserFiles([state.modelFiles[0], state.modelFiles[1]])
    ).then(model=>{
